...
const Page = () => {
  ...
  const favoriteProducts = React.useMemo(() => {
    ...
    return ids
      .map(
        (id) => productMap.get(id) || favorites.find((f) => String(f.product_id) === id)
      )
      .filter(Boolean);
    ...
    <>
      ...
      {productsError && (
        <div className="p-4 text-sm text-red-500">
          Lỗi tải dữ liệu: {String(productsError)}
        </div>
      )}
      {favoritesError && (
        <div className="p-4 text-sm text-red-500">
  ...
  if (isLaptop) {
    return (
      <div className="flex flex-col items-center gap-10 mt-10">
        <div className="w-[90%]">
          <p className="text-[32px] text-[#9B8D6F] text-left">Danh sách yêu thích</p>
        </div>
    ...
    return (
      <div className="flex flex-col items-center gap-8 px-6 pb-14 pt-8">
        <div className="w-full max-w-5xl">
          <p className="text-[28px] text-[#9B8D6F] text-left">Danh sách yêu thích</p>
        </div>
  ...
  if (isMobile) {
    return (
      <div className="flex flex-col gap-6 px-4 pb-14 pt-6">
        <p className="text-[22px] text-[#9B8D6F] text-left">Danh sách yêu thích</p>
        {content}
      </div>
    );
  }
